...
def sigmoid_model_3p(x, L, k, x0):
    return L / (1 + np.exp(-k * (x - x0)))

def analyze_sigmoid_3p():
    # Загрузка данных
    df = pd.read_csv("../datasets/Reg_Sigmoid.csv")
    X = df["X"].values
    Y = df["Y"].values
    ...
    # Границы параметров для trf
    bounds = (
        [0, 0, min(X)],  # Нижние границы: L, k, x0
        [np.inf, np.inf, max(X)]  # Верхние границы
    )

    # Аппроксимация с методом 'trf'
    popt, pcov = curve_fit(
        sigmoid_model_3p, X, Y,
        p0=initial_guess,
        bounds=bounds,
        method='trf',
        maxfev=10000
    ...
    # Прогноз и остатки
    Y_pred = sigmoid_model_3p(X, *popt)
    residuals = Y - Y_pred
