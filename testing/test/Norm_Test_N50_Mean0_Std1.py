...
def chi_square_normality_test(data):
    """Проверка нормальности распределения с помощью критерия хи-квадрат"""
    n = len(data)
    ...
    # Степени свободы = число групп – число оценённых параметров – 1
...
def main():
    file_path = os.path.join("..", "datasets", "Norm_Test_N50_Mean0_Std1.csv")
    ...
    if p_chi is None:
        print("▶ χ²: недостаточно интервалов для теста (df ≤ 0)\n")
    else:
        print("▶ χ²-критерий:")
